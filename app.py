...
st.markdown("""
    <div class="hero" style="padding-top:16px">
      <h1>🌙 달 & 🪐 태양계 통합 챗봇</h1>
      <p>왼쪽에서 챗봇을 선택하고, 빠른 질문 또는 채팅 입력으로 질문해보세요.</p>
    </div>
...
with st.sidebar:
    bot_choice = st.radio(
        "챗봇을 선택하세요",
        ["달 챗봇", "태양계 챗봇"],
        index=0 if st.session_state["selected_bot"] == "달 챗봇" else 1,
        key="bot_radio"
    )
    if bot_choice != st.session_state["selected_bot"]:
        st.session_state["selected_bot"] = bot_choice

    st.markdown("----")
    st.markdown(f"<div class='info-box'><b>🗓️ 오늘의 정보</b><br>날짜 : {date_str}</div>", unsafe_allow_html=True)

# === 2. 챗봇별 메인구성 ===
if st.session_state["selected_bot"] == "달 챗봇":
    conf = {
        "key": "moon_history",
...
user_text = st.chat_input("궁금한 걸 입력해 보세요!", key=conf["input_key"])
if user_text:
    if conf["key"] == "moon_history" and not conf["check_topic"](user_text):
...
if st.session_state[history_key] and st.session_state[history_key][-1]["role"] == "user":
    with st.spinner(conf["spinner"]):
        ...
        st.markdown(
            f"<div class='bot-bubble'>{avatar} {ai_text}</div>",
            unsafe_allow_html=True,
        )
